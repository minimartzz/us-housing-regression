...
def add_date_features(df: pd.DataFrame) -> pd.DataFrame:
  """Splits date feature into year, quarter and month

  Args:
      df (pd.DataFrame): Data for preprocessing

  Returns:
      pd.DataFrame: Dataframe with separated date information
  """
...
def frequency_encode(train: pd.DataFrame, eval: pd.DataFrame, col: str):
  """Frequency encoding on column learned on train data, evaluated on validation data

  Args:
      train (pd.DataFrame): Train split
      eval (pd.DataFrame): Validation split
      col (str): Column to apply to
...
def run_feature_engineering(
  ...
  if in_eval_path is None:
    in_eval_path = PROCESSED_DIR / "cleaning_valid.csv"
  ...
  out_eval_path = output_dir / "feature_engineered_valid.csv"
  out_holdout_path = output_dir / "feature_engineered_test.csv"
